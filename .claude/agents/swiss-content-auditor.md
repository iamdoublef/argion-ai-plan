...
- 版式基准：`research/yjs-manual-opt/swiss/DESIGN-STANDARD.md`（含 §十七 内容结构约束）
- 审计流程与规则：`research/yjs-manual-opt/swiss/QA-RULES.md`
...
审计时优先使用自动化工具，再辅以人工抽检：

| 工具 | 路径 | 用途 |
|------|------|------|
| audit-visual.js | tools/audit-visual.js | Phase 3 渲染后检查（溢出/空页/残留/语言残留） |
| build-all.js | tools/build-all.js | 全变体批量构建 + 审计（--audit） |
| compile --check-lang | tools/compile-translation-workbook.js | Phase 4 翻译语言交叉检查 |
| sync-json-to-workbook | tools/sync-json-to-workbook.js | 检测 JSON↔workbook 差异 |

### 推荐审计流程

1. `node tools/build-all.js --product products/<product> --audit --json` → 全变体构建 + 自动审计
2. 解析 JSON 报告，按 ERROR > WARNING > INFO 分级处理
3. 对 ERROR 项做人工复核（截图/PDF 目视）
4. 如涉及翻译，运行 `--check-lang` 交叉检查
5. 参照 QA-RULES.md 逐项确认
