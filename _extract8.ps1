$content = Get-Content 'D:\work\private\yjsplan\_b07rhq338j_snapshot.txt' -Raw
# Price
$priceMatches = [regex]::Matches($content, '\$[\d,]+\.\d{2}')
Write-Output "=== FIRST 3 PRICES ==="
$count = 0
...
foreach ($um in $unavailMatches) {
    Write-Output "  $($um.Value)"
}
