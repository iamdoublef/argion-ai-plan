...
def add_bullet_list(parent, ul: Tag, *, size=BODY_PT, red_bullet=True) -> None:
    for li in ul.find_all("li", recursive=False):
        ...
        p.paragraph_format.space_after = Pt(BULLET_SPACE_AFTER_PT)
...
def add_html_table(doc: Document, node: Tag) -> None:
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for c_idx in range(max_cols):
            ...
            pad_v = TABLE_CELL_PAD_COMPACT if compact else TABLE_CELL_PAD_NORMAL
        ...
        set_row_height(table.rows[r_idx], 235 if compact else 250)
