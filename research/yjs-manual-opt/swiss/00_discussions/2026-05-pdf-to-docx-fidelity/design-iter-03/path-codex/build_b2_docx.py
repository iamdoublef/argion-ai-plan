...
def add_bullet_list(parent, ul: Tag, *, size=BODY_PT, red_bullet=True, tight=False) -> None:
    for li in ul.find_all("li", recursive=False):
        p = parent.add_paragraph()
        p.paragraph_format.left_indent = Mm(3.2)
...
def add_alert_box(doc: Document, node: Tag, *, compact_safety=False) -> None:
    ...
    set_cell_margins(cell, top=ALERT_CELL_PAD_V, start=110, bottom=ALERT_CELL_PAD_V, end=110)
    if fill != "FFFFFF":
    ...
    if title:
        ...
        if not has_real_icon and not compact_safety:
    ...
    for child in node.children:
        ...
        if child == title or child.name == "img":
            continue
        if child.name == "ul":
            add_bullet_list(cell, child, size=6.98 if compact_safety else BODY_PT, tight=compact_safety)
        elif child.name == "p":
