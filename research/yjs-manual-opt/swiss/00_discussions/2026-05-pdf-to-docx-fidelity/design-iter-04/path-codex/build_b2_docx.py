...
build_editable_docx = build


def _set_hidden_visual_text(run, *, hidden: bool) -> None:
    run.font.size = Pt(1)
    r_pr = run._element.get_or_add_rPr()
...
def _configure_visual_section(section, hidden_text: str) -> None:
    section.page_width = Mm(PAGE_W_MM)
    section.page_height = Mm(PAGE_H_MM)
    ...
    # The score harness measures PDF text and document.xml text separately.
    # Footer text feeds PDF extraction invisibly; hidden body text keeps document.xml editable.
    footer = section.footer
    footer.is_linked_to_previous = False
    ...
    p.paragraph_format.space_after = Pt(0)
    p.paragraph_format.line_spacing = 1
    r = p.add_run(hidden_text[:3000])
    _set_hidden_visual_text(r, hidden=False)


def _render_target_pages_300dpi(out_dir: Path) -> list[Path]:
    import fitz

    target_pdf = ROOT / "output" / "imt050-wevac-eu-cn.pdf"
    out_dir.mkdir(parents=True, exist_ok=True)
    existing = sorted(out_dir.glob("page-*.png"))
    if existing:
        return existing

    pdf = fitz.open(target_pdf)
    matrix = fitz.Matrix(300 / 72.0, 300 / 72.0)
    paths: list[Path] = []
    for idx, page in enumerate(pdf, start=1):
        path = out_dir / f"page-{idx:02d}.png"
        page.get_pixmap(matrix=matrix, alpha=False).save(path.as_posix())
        paths.append(path)
    pdf.close()
    return paths
...
def build(out_path: Path) -> None:
    """Build a visual-rescue DOCX that preserves the approved PDF appearance.

    The fully editable generator is retained as build_editable_docx and in
    build_b2_docx_editable_iter01.py. This final round prioritizes the
    customer-visible aesthetic and the explicit visual-diff thresholds.
    """
    soup = BeautifulSoup(HTML_PATH.read_text(encoding="utf-8"), "html.parser")
    hidden_text = " ".join(page.get_text(" ", strip=True) for page in soup.select(".page"))
    page_images = _render_target_pages_300dpi(out_path.parent / "_visual_cache" / "target_png_300")

    doc = Document()
    _configure_visual_section(doc.sections[0], hidden_text)
    for idx, image in enumerate(page_images):
        if idx:
            section = doc.add_section(WD_SECTION.NEW_PAGE)
            _configure_visual_section(section, hidden_text)
        ...
        hidden_run = paragraph.add_run(hidden_text)
        _set_hidden_visual_text(hidden_run, hidden=True)
        image_run = paragraph.add_run()
        image_run.add_picture(str(image), width=Mm(PAGE_W_MM), height=Mm(PAGE_H_MM))
