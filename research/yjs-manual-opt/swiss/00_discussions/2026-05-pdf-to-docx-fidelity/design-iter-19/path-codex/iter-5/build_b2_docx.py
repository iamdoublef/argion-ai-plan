...
def set_table_indent(table, indent_mm: float) -> None:
    tbl_pr = table._tbl.tblPr
    tbl_ind = tbl_pr.find(qn("w:tblInd"))
    if tbl_ind is None:
        tbl_ind = OxmlElement("w:tblInd")
        tbl_pr.append(tbl_ind)
    tbl_ind.set(qn("w:w"), str(int(indent_mm * 56.7)))
...
def add_image_row(parent, node: Tag, *, max_h_override=None) -> None:
    ...
    if "step-figures" in node.get("class", []) and len(imgs) == 2:
        ...
        set_table_width(table, 78.5)
        set_table_grid(table, [36.0, 42.5])
        set_table_indent(table, 33.0)
        remove_table_borders(table)
        for i, img in enumerate(imgs):
            ...
            if not path.exists():
                continue
            cell = table.cell(0, i)
            set_cell_width(cell, 36.0 if i == 0 else 42.5)
