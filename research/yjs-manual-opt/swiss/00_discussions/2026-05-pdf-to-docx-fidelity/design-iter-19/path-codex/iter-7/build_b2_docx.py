...
def add_section_title(doc: Document, node: Tag, *, title_size=CHAPTER_TITLE_PT) -> None:
    p = doc.add_paragraph()
    ...
    r2 = p.add_run(title)
    set_run_font(r2, size=title_size, bold=True, color=BLACK)
...
def add_html_table(doc: Document, node: Tag, *, compact_warranty=False, troubleshooting=False) -> None:
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for c_idx in range(max_cols):
            ...
            pad_v = 42 if troubleshooting and compact else (52 if compact_warranty and not compact else (TABLE_CELL_PAD_COMPACT if compact else 36))
...
def add_body_page(doc: Document, page: Tag) -> None:
    ...
    for child in page.find_all(recursive=False):
        ...
        if "header-strip" in classes:
            ...
            title_size = CHAPTER_NUM_PT if "compact-warranty" in page_classes else CHAPTER_TITLE_PT
            add_section_title(doc, child, title_size=title_size)
