...
VARIANT = os.environ.get("B_VARIANT", "grid-all")
...
def variant_enabled(name: str) -> bool:
    tokens = {token.strip().lower() for token in re.split(r"[,;+]", VARIANT.lower()) if token.strip()}
...
def set_table_width(table, width_mm: float = CONTENT_W_MM) -> None:
    ...
    if variant_enabled("grid-all"):
        existing = list(tbl.tblGrid)
        col_count = len(existing)
        if col_count == 0 and len(tbl.tr_lst) > 0:
            col_count = len(tbl.tr_lst[0].tc_lst)
        if col_count > 0:
            set_table_grid(table, [width_mm / col_count] * col_count)
...
def set_run_fonts(run, latin=LATIN_FONT, east_asia=CJK_FONT, latin_bold=LATIN_BOLD_FONT, is_bold=False, is_mono=False):
    ...
    if variant_enabled("eastasia-hint-all"):
        rFonts.set(qn("w:hint"), "eastAsia")


def set_eastasia_hint_for_cjk(run) -> None:
    if not variant_enabled("eastasia-hint-cn"):
        return
    if not re.search(r"[\u3400-\u9fff]", run.text or ""):
        return
...
def embed_image_path(path: Path) -> Path:
    if not variant_enabled("jpg-300"):
        return path
    cache = SCRIPT_DIR / "_image_cache_jpg300"
    ...
    with Image.open(path) as im:
        if im.mode in ("RGBA", "LA"):
            ...
            im = im.convert("RGB")
        im.save(out, "JPEG", quality=96, optimize=True, dpi=(300, 300))
    return out


def convert_last_picture_to_anchor(run) -> None:
    if not variant_enabled("anchor-drawings"):
        return
    ...
    inline.insert(insert_at + 1, pos_h)
    inline.insert(insert_at + 2, pos_v)
    extent_idx = next((i for i, child in enumerate(inline) if child.tag == qn("wp:extent")), None)
    effect_idx = next((i for i, child in enumerate(inline) if child.tag == qn("wp:effectExtent")), None)
    inline.insert((effect_idx if effect_idx is not None else extent_idx or 0) + 1, wrap)
...
def add_image_paragraph(parent, img: Tag, *, align=WD_ALIGN_PARAGRAPH.CENTER, force_w=None, force_h=None):
    ...
    run = p.add_run()
    run.add_picture(str(embed_image_path(path)), width=Mm(w_mm), height=Mm(h_mm))
    convert_last_picture_to_anchor(run)
    return p
...
def add_alert_box(doc: Document, node: Tag, *, compact_safety=False) -> None:
    ...
    for img in node.find_all("img"):
        ...
        icon_w = 5.2 if compact_safety else 8
    ...
    if "status-indicator-row" in node.get("class", []):
        ...
        for child in children:
            if child.name == "span":
                ...
                if path.exists():
                    run = p.add_run()
                    run.add_picture(str(embed_image_path(path)), height=Mm(4))
                    convert_last_picture_to_anchor(run)
                p.add_run(" ")
        return
...
def apply_section_properties(section) -> None:
    if not variant_enabled("sectpr-docgrid"):
        return
    sect_pr = section._sectPr
    pg_sz = sect_pr.find(qn("w:pgSz"))
    if pg_sz is not None:
        pg_sz.set(qn("w:w"), str(int(PAGE_W_MM * 56.7)))
        pg_sz.set(qn("w:h"), str(int(PAGE_H_MM * 56.7)))
...
def configure_section(section, page_no: int | None = None) -> None:
    ...
    apply_section_properties(section)
