...
WORD_TUNE = os.environ.get("WORD_TUNE", "none")


def word_tune_enabled(*names: str) -> bool:
    active = {part.strip() for part in WORD_TUNE.split(",") if part.strip()}
    return any(name in active for name in names)


def set_on_off(parent, tag: str, val: str = "0") -> None:
...
def set_word_body_paragraph_compat(paragraph) -> None:
    if not word_tune_enabled("bodycompat", "all"):
        return
    p_pr = paragraph._p.get_or_add_pPr()
    set_on_off(p_pr, "w:adjustRightInd", "0")
    set_on_off(p_pr, "w:autoSpaceDE", "0")
    set_on_off(p_pr, "w:autoSpaceDN", "0")
...
def set_run_fonts(run, latin=LATIN_FONT, east_asia=CJK_FONT, latin_bold=LATIN_BOLD_FONT, is_bold=False, is_mono=False):
    ...
    if word_tune_enabled("eastasia", "all"):
...
def set_run_font(run, size=BODY_PT, bold=False, color=BLACK, is_mono=False) -> None:
    ...
    if word_tune_enabled("eastasia", "szcs_down", "szcs_up", "all"):
        half_points = int(round(size * 2))
        if word_tune_enabled("szcs_down"):
            half_points = max(1, half_points - 1)
        elif word_tune_enabled("szcs_up"):
            half_points += 1
    ...
    if word_tune_enabled("kern", "all"):
        kern = run._element.rPr.find(qn("w:kern"))
        if kern is None:
            kern = OxmlElement("w:kern")
            run._element.rPr.append(kern)
        kern.set(qn("w:val"), "2")
...
def compact_paragraph(paragraph, size=BODY_PT, after=2, before=0, line=BODY_LINE_SPACING) -> None:
    ...
    set_word_body_paragraph_compat(paragraph)
...
def add_bullet_list(
    ...
    *,
    ...
    for li in ul.find_all("li", recursive=False):
        ...
        set_word_body_paragraph_compat(p)
        bullet = p.add_run("•    ")
...
def add_alert_box(doc: Document, node: Tag, *, compact_safety=False) -> None:
    ...
    for img in node.find_all("img"):
        ...
        if p_img:
            ...
            set_word_body_paragraph_compat(p_img)
    for child in node.children:
...
def add_html_table(doc: Document, node: Tag, *, compact_warranty=False, troubleshooting=False) -> None:
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for c_idx in range(max_cols):
            ...
            end_pad = 55
            if word_tune_enabled("tcmar_tight", "all") and (compact_warranty or troubleshooting):
                pad_v = max(20, pad_v - 8)
                start_pad = max(42, start_pad - 12)
                end_pad = max(42, end_pad - 8)
            set_cell_margins(cell, top=pad_v, start=start_pad, bottom=pad_v, end=end_pad)
            if is_warranty_card:
