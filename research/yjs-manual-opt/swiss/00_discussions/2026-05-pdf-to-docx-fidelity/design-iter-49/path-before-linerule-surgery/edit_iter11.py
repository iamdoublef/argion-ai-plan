"""iter-11: p9 surgical — line=264 -> line=240 on p9 cohort (idx 169..183).
Snap to body cohort (-24 twips). Push iter-10 further.
"""
import re
from pathlib import Path

ROOT = Path(__file__).parent
doc_path = ROOT / "iter-11" / "unpacked" / "word" / "document.xml"
...
for i, m in enumerate(p_pat.finditer(doc)):
    ...
    body = m.group(0)
    if i in target_idx:
        new_body, n = re.subn(r'(<w:spacing[^/]*?)w:line="264"', r'\1w:line="240"', body)
        if n > 0:
            count_modified += 1
            body = new_body
