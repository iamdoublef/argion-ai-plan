"""iter-11: stack iter-10 + p10 val=2->0 (max compression)."""
import re
import shutil
import sys
...
def apply(iter_name: str):
    ...
    # p10: 10/8/6/4 all collapse, then val=2 (the original 2 plus the just-moved 4s) -> 0
    ...
    # NEW: val=2 -> 0
    n2 = new_page.count('<w:spacing w:val="2"/>')
    new_page = new_page.replace('<w:spacing w:val="2"/>', '<w:spacing w:val="0"/>')
    text = text[:s] + new_page + text[e:]

    doc.write_text(text, encoding="utf-8")
    print(f"done. iter-10 stack + p10 val=2->0: {n2} sites (all p10 spacing now 0)")
    return 0
