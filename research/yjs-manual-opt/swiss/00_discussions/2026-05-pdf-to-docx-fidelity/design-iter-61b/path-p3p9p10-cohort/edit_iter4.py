"""iter-4: stack iter-2 + p3 val=9->11 (upward direction)."""
import re
import shutil
import sys
...
def apply(iter_name: str):
    ...
    # NEW: p3 val=9->11 (upward)
    s, e = page_span(secs, 3, len(text))
    page = text[s:e]
    n = page.count('<w:spacing w:val="9"/>')
    new_page = page.replace('<w:spacing w:val="9"/>', '<w:spacing w:val="11"/>')
    text = text[:s] + new_page + text[e:]

    doc.write_text(text, encoding="utf-8")
    print(f"done. iter-2 stack + p3 val=9->11: {n} sites")
    return 0
