"""iter-3: p14 line=250 -> 240 (21 sites, DOWN direction).

iter-2 line=250->260 = p14 +0.47 BAD. Try opposite: tighten line.
"""
import re
import shutil
import sys
...
def apply(iter_name: str):
    ...
    # p14: line="250" -> "240" (21 sites)
    s, e = page_span(secs, 14, len(text))
    page = text[s:e]
    n = page.count('w:line="250"')
    new_page = page.replace('w:line="250"', 'w:line="240"')
    text = text[:s] + new_page + text[e:]

    doc.write_text(text, encoding="utf-8")
    print(f"done. p14 line=250->240: {n} sites")
    return 0
