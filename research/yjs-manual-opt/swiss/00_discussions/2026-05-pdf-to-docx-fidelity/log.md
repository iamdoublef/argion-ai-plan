...
## 2026-05-17 18:40 · design-iter-61b · W49 — 7.23/10.13

**操作**：在 W48 (7.27/10.13, iter-60) 基础上攻 p3/p9/p10 + 未触 cohort。
工作目录 `design-iter-61b/path-p3p9p10-cohort/`。

**iter 概要**（14 个）：

| iter | 改动 | mean | max | 结论 |
|---|---|---|---|---|
| 1 | iter-14_61 + p3 v6→4 | 7.35 | 11.44 | ❌ p3 BLOW UP |
| 2 | iter-14_61 + p10 v10→6 (深) | 7.25 | 10.13 | ✅ p10 9.69 |
| 3 | iter-2 + p3 v9→7 | 7.34 | 11.44 | ❌ p3 BLOW UP |
| 4 | iter-2 + p3 v9→11 | 7.37 | 11.84 | ❌ p3 两向饱和 |
| 5 | iter-2 + p3 line=228→232 | 7.42 | 12.67 | ❌ p3 line 饱和 |
| 6 | iter-2 + p9 v10→8 | 7.29 | 10.39 | ❌ p9 回退 |
| 7 | iter-2 + p10 v8→6 | 7.25 | 10.13 | ✅ 边际 |
| 8 | iter-7 + p9 v5→3 | 7.25 | 10.13 | ❌ p9 微回退 |
| 9 | iter-7 + p10 v6→4 级联 | 7.24 | 10.13 | ✅ p10 9.55 突破 |
| 10 | iter-9 + p10 v4→2 | 7.24 | 10.13 | ✅ p10 9.55 |
| 11 | iter-10 + p10 v2→0 | 7.23 | 10.13 | ✅ p10 9.48 |
| **12** | **iter-11 + p9 v6→4** | **7.23** | **10.13** | **✅ BEST 升级** |
| 13 | iter-12 + p9 v10→8 | 7.27 | 10.35 | ❌ p9 回退 |
| 14 | iter-12 + after=120→100 | 7.84 | 12.00 | ❌ 灾难，禁用 |

**关键发现**：
1. **p10 所有 spacing 值级联崩塌至 0** 是最大突破 — 每一档 (10→6→4→2→0) 都降低 p10 diff
2. **p3 char-spacing 完全饱和** — val=6/val=9 双向都炸，line=228 也炸；p3 留给后续创意角度
3. **p9 val=2→0、8→6、6→4 累积可降 0.25**，但 val=10、val=5 触碰即回退
4. **全局 w:after=120 (35 sites) 是禁区** — 在 W48 上加该 cohort 平均涨 0.6

**Final 升级**：
- W48 → W49 = 7.23/10.13（mean -0.04，max 不变）
- p9 9.94→9.69, p10 9.79→9.48
- wt_count=448, Word-safe, all PASS

**新增饱和清单（METHODOLOGY.md 应补）**：
- p3 val=6/val=9/line=228（任何方向都炸）
- p9 val=10、val=5（任何 ±2 都回退）
- 全局 after=120 全局回退

**下一推荐**：
- p9 line=244 (7 sites)、p10 line=228 (4 sites) 未触线距 cohort
- p11 仍为 max 瓶颈 10.13，需创新 lever（trHeight/banner 已饱和，剩 cellMar 内距子方向）
- p14=9.67 从未单页攻过，可能易得分
- p3 仍是次高点 (10.03)，需切入"分段落-非整-cohort"思路（例如仅 p3 表格 cellMar）
