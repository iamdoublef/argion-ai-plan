...
const DOCX_PROFILE = {
  ...
  text: {
    ...
    chapterNumberSize: 30,  // big red 01/02, was 18pt
    chapterTitleSize: 28,   // chapter title, was 17pt
...
function renderChapterHeading(chapter) {
  // iter-01: drop the extra header_ref paragraph (already in page header).
  // Tight spacing so content follows immediately, like the PDF.
  return [
    new Paragraph({
      heading: HeadingLevel.HEADING_1,
      spacing: { before: 0, after: 120, line: 260 },
      border: { left: { style: BorderStyle.SINGLE, size: 18, color: ACTIVE_THEME.chapterBar } },
      indent: { left: 140 },
      keepNext: true,
      children: [
        new TextRun({
...
function buildCoverBlock(ctx) {
  ...
  // Big vertical breathing room in the middle (cover image sits ~mid page)
