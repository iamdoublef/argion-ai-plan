...
function renderChapterHeading(chapter) {
  // iter-02: chapter bar uses brand accent (red). Paragraph border size is in
  // 1/8 pt — size 80 ≈ 10pt thick to read as a solid red block.
  return [
    new Paragraph({
...
function buildCoverBlock(ctx) {
  ...
  // iter-02: push the product image down toward middle of the cover.
  // Each empty paragraph with line=240 = ~12pt ≈ 4mm of vertical space.
  // Tuned so we keep total cover height under one A5 page.
  for (let i = 0; i < 5; i += 1) {
