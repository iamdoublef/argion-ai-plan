...
C_GRAY_BG = RGBColor(0xF2, 0xF2, 0xF7)
C_GRAY_TEXT = RGBColor(0x8E, 0x8E, 0x93)
C_DARK_TEXT = RGBColor(0x22, 0x22, 0x22)

# 中文字体优先 Microsoft YaHei；西文 Helvetica/Arial
...
def set_run_fonts(run, *, cn_font=FONT_CN, latin_font=FONT_LATIN, size_pt: Optional[float] = None,
                  bold: bool = False, color: Optional[RGBColor] = None, mono: bool = False) -> None:
    if mono:
        cn_font = FONT_MONO
...
def parse_inline_md(paragraph, text: str, *, default_size: float = 9.5,
                    color: Optional[RGBColor] = C_DARK_TEXT, mono_kw: bool = False) -> None:
...
def set_page_size(section, w_mm: float, h_mm: float, margin_mm: float = 10.0,
                  header_mm: float = 4.0, footer_mm: float = 4.0) -> None:
...
class ManualBuilder:
    ...
    def build(self):
        ...
        # Wipe default Normal style spacing so things are tight
    ...
    def _setup_body_header_footer(self, section):
        # HEADER: top border (black 4pt), brand left, chapter ref right
        ...
        set_paragraph_border(p, top=(24, "000000"))
        # Tabs: left at 0, right at page_width - 2*margin
        ...
        r1 = p.add_run("威富可")
        set_run_fonts(r1, size_pt=8.5, bold=True, color=C_BLACK)
        p.add_run("\t")
        r2 = p.add_run("CHAPTER_REF_PLACEHOLDER")
        set_run_fonts(r2, size_pt=7, color=C_GRAY_TEXT, mono=True)

        # FOOTER: brand IMT050 ... page no on right
        footer = section.footer
        for p in list(footer.paragraphs):
            p._p.getparent().remove(p._p)
        p = footer.add_paragraph()
        set_paragraph_spacing(p, before_pt=2, after_pt=0)
        pf = p.paragraph_format
        pf.tab_stops.add_tab_stop(Mm(PAGE_W_MM - 2 * PAGE_MARGIN_MM),
                                  alignment=WD_TAB_ALIGNMENT.RIGHT)
        rf1 = p.add_run("威富可  IMT050  说明书")
        set_run_fonts(rf1, size_pt=7.5, color=C_GRAY_TEXT)
        p.add_run("\t")
        # PAGE field
        run = p.add_run()
        set_run_fonts(run, size_pt=7.5, color=C_GRAY_TEXT)
    ...
    def _add_cover(self):
        """Cover layout:
        - Top: red dash + 威富可 (brand strip)
        - Middle: product image + MODEL IMT050 + 制冰机 + 说明书 + red divider
        - Bottom (in section FOOTER, not body): disclaimer

        Forcing disclaimer into footer guarantees it stays on the cover page.
        """
        sec = self.doc.sections[0]
        # tighter margins for cover so image breathes
        sec.top_margin = Mm(12)
        sec.left_margin = Mm(14)
        sec.right_margin = Mm(14)
        sec.bottom_margin = Mm(28)  # leave room for footer disclaimer block

        # cover brand strip: ━━  威富可
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.0)
        r1 = p.add_run("━━  ")
        set_run_fonts(r1, size_pt=9, bold=True, color=C_RED)
        r2 = p.add_run("威富可")
        set_run_fonts(r2, size_pt=10, bold=True, color=C_BLACK)
        rPr = r2._r.get_or_add_rPr()
        sp = OxmlElement("w:spacing")
        sp.set(qn("w:val"), "30")
        rPr.append(sp)

        # Use smaller spacers to push image down to ~middle of page
        for _ in range(5):
        ...
        if cover_path.exists():
            ...
            set_paragraph_spacing(p_img, before_pt=0, after_pt=2, line_spacing=1.0)
            p_img.alignment = WD_ALIGN_PARAGRAPH.LEFT
            run = p_img.add_run()
            run.add_picture(str(cover_path), width=Mm(42))

        # MODEL IMT050
        p_model = self.doc.add_paragraph()
        set_paragraph_spacing(p_model, before_pt=6, after_pt=2, line_spacing=1.0)
        rm = p_model.add_run("MODEL  IMT050")
        set_run_fonts(rm, size_pt=9, bold=True, color=C_RED)
        ...
        set_paragraph_spacing(p_title, before_pt=0, after_pt=0, line_spacing=1.0)
        rt = p_title.add_run("制冰机")
        set_run_fonts(rt, size_pt=26, bold=True, color=C_BLACK)

        # Subtitle 说明书
        p_sub = self.doc.add_paragraph()
        set_paragraph_spacing(p_sub, before_pt=2, after_pt=4, line_spacing=1.0)
        rs = p_sub.add_run("说明书")
        set_run_fonts(rs, size_pt=10.5, color=C_GRAY_TEXT)

        # Red short divider (short bar)
        p_div = self.doc.add_paragraph()
        set_paragraph_spacing(p_div, before_pt=2, after_pt=0, line_spacing=1.0)
        pf = p_div.paragraph_format
        pf.right_indent = Mm(PAGE_W_MM - 2*14 - 18)
        set_paragraph_border(p_div, bottom=(24, "E63946"))
        r_div = p_div.add_run("")
        set_run_fonts(r_div, size_pt=2)

        # Set up the cover footer to hold the disclaimer
        cover_footer = sec.footer
        cover_footer.is_linked_to_previous = False
        # clear default
        for p in list(cover_footer.paragraphs):
            p._p.getparent().remove(p._p)

        # Top border line (above disclaimer)
        p_top = cover_footer.add_paragraph()
        set_paragraph_spacing(p_top, before_pt=0, after_pt=0, line_spacing=1.0)
        set_paragraph_border(p_top, top=(8, "000000"))
        p_top.add_run("")

        p_dis1 = cover_footer.add_paragraph()
        set_paragraph_spacing(p_dis1, before_pt=2, after_pt=0, line_spacing=1.3)
        rd1 = p_dis1.add_run("使用产品前请仔细阅读本说明书，并妥善保管。")
        set_run_fonts(rd1, size_pt=7.5, color=C_BLACK)
        p_dis2 = cover_footer.add_paragraph()
        set_paragraph_spacing(p_dis2, before_pt=0, after_pt=0, line_spacing=1.3)
        rd2 = p_dis2.add_run("说明书中的产品、配件等插图均为示意图，仅供参考。由于产品的更新与升级，产品实物与示意图可能略有差异，请以实物为准。")
        set_run_fonts(rd2, size_pt=7.5, color=C_BLACK)

    # ── TOC ────────────────────────────────────────────────────────
    def _add_toc(self):
        # Update header for body section: this page should show "IMT050 — 说明书"
        body_sec = self.doc.sections[1]
        self._update_chapter_ref_header(body_sec, "IMT050 — 说明书")

        # 目录 title
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=4, after_pt=8, line_spacing=1.05)
        rt = p.add_run("目录")
        set_run_fonts(rt, size_pt=24, bold=True, color=C_BLACK)
    ...
    def _add_toc_entry(self, num: str, title: str, page: int):
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=4, after_pt=4, line_spacing=1.3)
        # Right tab stop for page number with leader
        right_tab = Mm(PAGE_W_MM - 2 * PAGE_MARGIN_MM)
        pf = p.paragraph_format
        pf.tab_stops.add_tab_stop(right_tab, alignment=WD_TAB_ALIGNMENT.RIGHT)
        # bottom border under the row (subtle gray)
        set_paragraph_border(p, bottom=(4, "E5E5EA"))

        r_num = p.add_run(num)
        set_run_fonts(r_num, size_pt=10, bold=True, color=C_RED)
        r_sep = p.add_run("    ")
        set_run_fonts(r_sep, size_pt=10)
        r_title = p.add_run(title)
        set_run_fonts(r_title, size_pt=11, bold=True, color=C_BLACK)
        p.add_run("\t")
        r_page = p.add_run(str(page))
        set_run_fonts(r_page, size_pt=9, color=C_GRAY_TEXT)
    ...
    def _add_section_title(self, chapter_no: str, title: str, is_continued: bool):
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=0, after_pt=4, line_spacing=1.05)
        # left "block" bar via paragraph left border thick black
        set_paragraph_border(p, left=(28, "000000"))
        pf = p.paragraph_format
        pf.left_indent = Mm(2)
        # chapter num red
        r_num = p.add_run(chapter_no + " ")
        set_run_fonts(r_num, size_pt=15, bold=True, color=C_RED)
        # title black
        r_t = p.add_run(title)
        set_run_fonts(r_t, size_pt=13, bold=True, color=C_BLACK)
    ...
    def _render_paragraph(self, block: Dict[str, Any]):
        ...
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=1, after_pt=2, line_spacing=1.3)
        parse_inline_md(p, text, default_size=9, color=C_DARK_TEXT)

    def _render_sub_title(self, block: Dict[str, Any]):
        text = self.strings.block_text(block)
        p = self.doc.add_paragraph()
        set_paragraph_spacing(p, before_pt=4, after_pt=2, line_spacing=1.1)
        set_paragraph_border(p, bottom=(8, "000000"))
        r = p.add_run(text)
        set_run_fonts(r, size_pt=9.5, bold=True, color=C_BLACK)
    ...
    def _render_bullet_list(self, block: Dict[str, Any]):
        for item in block.get("items", []):
            ...
            r0 = p.add_run("•  ")
            set_run_fonts(r0, size_pt=9, bold=True, color=C_RED)
            parse_inline_md(p, text, default_size=8.7, color=C_DARK_TEXT)

    def _render_box(self, block: Dict[str, Any], *, kind: str):
        # 1×1 table, full width, with shading + thin red/gray border
        # title: prefer title_id from strings, fallback to block.title
        ...
        # body text: prefer text_id from strings, fallback to block.text
        ...
        first_para_pf = first_para.paragraph_format
        first_para_pf.space_before = Pt(0)
        first_para_pf.space_after = Pt(0)
        # use it for title; if block has an icon, render icon inline before title
        icon_id = block.get("icon")
        if icon_id:
            icon_path = self._image_path(icon_id)
            if icon_path and icon_path.exists():
                icon_run = first_para.add_run()
                try:
                    icon_run.add_picture(str(icon_path), height=Mm(3.5))
                except Exception:
                    pass
                first_para.add_run("  ")
        rt = first_para.add_run(title)
        set_run_fonts(rt, size_pt=8.5, bold=True, color=C_BLACK)
        set_paragraph_spacing(first_para, before_pt=0, after_pt=2, line_spacing=1.1)
        ...
        if items:
            for item in items:
                ...
                set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.22)
                pf = p.paragraph_format
                pf.left_indent = Mm(4)
                pf.first_line_indent = Mm(-3)
                r0 = p.add_run("•  ")
                set_run_fonts(r0, size_pt=8, bold=True, color=C_RED)
                parse_inline_md(p, t, default_size=8.3, color=C_DARK_TEXT)

        # body text (single paragraph) if no items but block has text
        if text and not items:
            p = cell.add_paragraph()
            set_paragraph_spacing(p, before_pt=1, after_pt=0, line_spacing=1.25)
            parse_inline_md(p, text, default_size=8.3, color=C_DARK_TEXT)

        # add a tiny spacer paragraph after the table (outside)
        sp = self.doc.add_paragraph()
    ...
    def _render_step_flow(self, block: Dict[str, Any]):
        ...
        for idx, step in enumerate(steps):
            ...
            tbl.rows[0].height = Mm(5.0)
            tbl.rows[0].height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
            # num cell
            c0 = tbl.cell(0, 0)
            c0.width = Mm(num_col_w)
            set_cell_shading(c0, "000000")
            set_cell_borders(c0,
                             top=(8, "000000"), bottom=(8, "000000"),
                             left=(8, "000000"), right=(8, "000000"))
            ...
            rn = p0.add_run(str(n))
            set_run_fonts(rn, size_pt=8.5, bold=True, color=RGBColor(0xFF, 0xFF, 0xFF))
            # text cell
            c1 = tbl.cell(0, 1)
            ...
            p1 = c1.paragraphs[0]
            set_paragraph_spacing(p1, before_pt=0, after_pt=0, line_spacing=1.25)
            parse_inline_md(p1, text, default_size=9, color=C_DARK_TEXT)
            # figures (if any)
    ...
    def _render_figure_row(self, block: Dict[str, Any]):
        ...
        if figs:
            ...
            for i, item in enumerate(items):
                ...
                r = p.add_run(label)
                set_run_fonts(r, size_pt=8.5, bold=True, color=C_DARK_TEXT)
                # image cell
    ...
    def _render_inline_table(self, block: Dict[str, Any]):
        ...
        for h in headers:
            ...
            if isinstance(w, str) and w.endswith("%"):
                p = float(w[:-1])
                widths_mm.append(usable_w * p / 100.0)
                pct_sum += p
            else:
        ...
        for ci, h in enumerate(headers):
            ...
            set_cell_shading(cell, "F2F2F7")
            set_cell_borders(cell,
                             top=(4, "D1D1D6"), bottom=(4, "D1D1D6"),
                             left=(4, "D1D1D6"), right=(4, "D1D1D6"))
            set_cell_margins(cell, top=30, bottom=30, left=60, right=60)
            txt = self.strings.get(h.get("text_id", ""), h.get("text", ""))
            p = cell.paragraphs[0]
            set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.2)
            r = p.add_run(txt)
            set_run_fonts(r, size_pt=8.7, bold=True, color=C_BLACK)
        # body rows
        for ri, row in enumerate(rows):
            for ci, cell_data in enumerate(row):
                ...
                cell.width = Mm(widths_mm[ci])
                set_cell_borders(cell,
                                 top=(4, "E5E5EA"), bottom=(4, "E5E5EA"),
                                 left=(4, "E5E5EA"), right=(4, "E5E5EA"))
                set_cell_margins(cell, top=30, bottom=30, left=60, right=60)
                if ri % 2 == 0:
                    set_cell_shading(cell, "FAFAFC")
                txt = self.strings.get(cell_data.get("text_id", ""), cell_data.get("text", ""))
                p = cell.paragraphs[0]
                set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.25)
                parse_inline_md(p, txt, default_size=8.5, color=C_DARK_TEXT)
    ...
    def _render_specs_table(self):
        ...
        for ci, label in enumerate([ui["specs_col1"], ui["specs_col2"]]):
            ...
            p = cell.paragraphs[0]
            set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.15)
            r = p.add_run(label)
            set_run_fonts(r, size_pt=8.7, bold=True, color=C_BLACK)
        # rows
        for ri, row in enumerate(specs_rows):
            ...
            for c in (cell0, cell1):
                ...
                set_cell_borders(c,
                                 top=(4, "E5E5EA"), bottom=(4, "E5E5EA"),
                                 left=(4, "E5E5EA"), right=(4, "E5E5EA"))
                set_cell_margins(c, top=20, bottom=20, left=70, right=70)
                # alternating row shading
                if ri % 2 == 0:
                    set_cell_shading(c, "FAFAFC")
                p = c.paragraphs[0]
                set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.2)
            r0 = cell0.paragraphs[0].add_run(row["label"])
            set_run_fonts(r0, size_pt=8.7, color=C_DARK_TEXT)
            r1 = cell1.paragraphs[0].add_run(row["value"])
            set_run_fonts(r1, size_pt=8.7, color=C_DARK_TEXT)

    def _render_parts_table(self):
        """Render structure parts as two-column-pairs table: [No][Name] x 2 per row."""
        parts = self.product["parts"]
        ui = self.product["ui_labels"]["cn"]
        usable_w = PAGE_W_MM - 2 * PAGE_MARGIN_MM
        # 4 cols: No | Name | No | Name
        c1_w = usable_w * 0.10
        c2_w = usable_w * 0.40
        c3_w = usable_w * 0.10
        c4_w = usable_w * 0.40
        ...
        for ci, lbl in enumerate(header_labels):
            ...
            r = p.add_run(lbl)
            set_run_fonts(r, size_pt=8.7, bold=True, color=C_BLACK)
        # rows — column-major pairing: row 1 has parts[0] & parts[rows]; row 2 has parts[1] & parts[rows+1] ...
        for ri in range(rows):
            ...
            for ci, (kind, part) in enumerate(entries):
                ...
                if kind == "id":
                    ...
                    set_run_fonts(r, size_pt=9, color=C_DARK_TEXT)
                else:
    ...
    def _render_buttons_table(self):
        ...
        for ri, b in enumerate(buttons):
            ...
            set_run_fonts(r_n, size_pt=9, color=C_DARK_TEXT)
            # key chip (monospace, inside [])
            r_key = p0.add_run(b["key"])
            set_run_fonts(r_key, size_pt=8, bold=True, color=C_BLACK, mono=True)
            r_sp = p0.add_run("   ")
            set_run_fonts(r_sp, size_pt=9)
            r_name = p0.add_run(b["name_cn"])
            set_run_fonts(r_name, size_pt=9, color=C_DARK_TEXT)
            p1 = cell1.paragraphs[0]
            r_desc = p1.add_run(b["desc_cn"])
            set_run_fonts(r_desc, size_pt=9, color=C_DARK_TEXT)
    ...
    def _render_two_col_kv_table(self, rows: List[tuple]):
        ...
        for ri, (label, value, bold) in enumerate(rows):
            ...
            set_run_fonts(r0, size_pt=9, color=C_DARK_TEXT)
            parse_inline_md(cell1.paragraphs[0], value, default_size=9, color=C_DARK_TEXT)
            if bold:
    ...
    def _render_warranty_card(self, block: Dict[str, Any]):
        ...
        for ri, field in enumerate(fields):
            ...
            for c, ww in ((cell0, col1_w), (cell1, col2_w)):
                ...
                # alternating row shading: even rows get subtle gray
                if ri % 2 == 0:
                    set_cell_shading(c, "F7F7FA")
                p = c.paragraphs[0]
                set_paragraph_spacing(p, before_pt=0, after_pt=0, line_spacing=1.3)
