...
> - **适用边界**：本节只写可复用的结构/渲染标准；产品私有分页经验应写入各产品目录下的 README
...
| C14 | `figure_row`、`split_panel` 这类多图/图文混排页，不得出现“图片过小但页面留白过大”或“同组图片明显一大一小”的失衡布局 | [渲染] | 若同页出现明显大面积空白或同组图片视觉重量失衡，应优先放大图片、调整承载方式或合并相邻相关内容，而不是保留弱可读缩图 |
| C15 | 同一章节连续两页如果后一页仍是同一主题的延续内容，前一页不得因默认分页留下大面积空白 | [渲染] | 常见于附件操作页、故障长表后续说明页；应优先重排到更紧凑但仍清晰的同页承载 |
| C16 | `warranty_info` 与 `warranty_card` 的分页必须以“信息完整 + 留白合理”为准，不得机械拆成两页 | [渲染] | 若两者同属一个连续保修范围且合并后可在同页稳定容纳，应优先同页；若确实过长再拆页 |
