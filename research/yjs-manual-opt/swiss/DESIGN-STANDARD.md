...
| C14 | `figure_row`、`split_panel` 这类多图/图文混排页，不得出现“图片过小但页面留白过大”的失衡布局 | [渲染] | 若同页出现明显大面积空白，应优先放大图片或合并相邻相关内容，而不是保留弱可读缩图 |
| C15 | 同一章节连续两页如果后一页仍是同一主题的延续内容，前一页不得因默认分页留下大面积空白 | [渲染] | 常见于附件操作页、故障长表后续说明页；应优先重排到更紧凑但仍清晰的同页承载 |
| C16 | `warranty_info` 与 `warranty_card` 的分页必须以“信息完整 + 留白合理”为准，不得机械拆成两页 | [渲染] | 若两者合并后可在同页稳定容纳，应优先同页；若确实过长再拆页 |
| C17 | 含 `rowspan` 的结构化表格，分类列跨行数必须与实际数据行数严格一致 | [结构] | 修改分类项或行数后必须同步检查 rowspan；禁止“视觉上差不多”的手工估算 |
| C18 | 同一产品线一旦确认中文批准版，后续地区/品牌变体不得漂移正文结构、分页逻辑和图文组织 | [结构] | 允许变化的仅限品牌字段、主题 token、本地化文本和法规/市场必需差异 |
| C19 | 同一变体中的正文单位表达必须与规格表单位保持一致 | [结构] | 单位更新视为同一任务：正文和规格表必须一起改，不允许双套口径并存 |
