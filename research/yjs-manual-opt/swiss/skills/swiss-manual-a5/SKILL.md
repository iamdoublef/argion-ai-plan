...
6. 审计流程与规则：
   - `research/yjs-manual-opt/swiss/QA-RULES.md`
7. 构建与输出工具：
   - `research/yjs-manual-opt/swiss/tools/build-variant.js`
   - `research/yjs-manual-opt/swiss/tools/build-all.js`（批量构建 + 审计）
   ...
   - `research/yjs-manual-opt/swiss/tools/compile-translation-workbook.js`（含 --check-lang）
   - `research/yjs-manual-opt/swiss/tools/sync-json-to-workbook.js`（JSON → xlsx 回写）
   - `research/yjs-manual-opt/swiss/tools/audit-visual.js`（渲染后视觉审计）
