...
| # | 陷阱 | 对策 |
|---|------|------|
| 1 | Flex 容器并排不同比例图片导致窄图拉伸 | `align-items:flex-start` 强制，`object-fit:contain` 单独不够 |
| 2 | 多语言模板正则匹配硬编码 notice title | 用 `[^<]+` 通配，不硬编码 NOTICE/HINWEIS/AVVISO |
| 3 | 插入新页面后批量更新页码误伤新页 | 占位符标记 → 旧页码更新 → 还原占位符 |
| 4 | `_audit-visual.js` 退出码不可靠 | 查 stdout `ALL CHECKS PASSED`，不用 exit code |
| 5 | `make-booklet.py --all --filter` 匹配不到 | 用 PowerShell 循环 + 单文件模式 |
